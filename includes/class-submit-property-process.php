			...
			if ( ! empty( $_POST[ 'process_property' ] ) ) {
				...
				if ( 'create_property' == $process_property ) {
					...
					if ( is_wp_error( $property_id ) ) {
						...
						$id_page_dashboard = RP_AddOn_Agent_Dashboard::get_url_agent_dashboard();

						RP_MemberShip::decrease_remaining_properties($agent_id);
						...
						$response[ 'url' ]     = apply_filters( 'rp_submit_property_redirect', $id_page_dashboard );
					}
					...
					foreach ( $_POST as $key => $value ) {
						...
						switch ( $key ) {
							case 'offers':
								...
								wp_set_object_terms( $property_id, $value, 'property_status' );
								break;

							case 'type' :
								$value = array( absint( $value ) );
								wp_set_object_terms( $property_id, $value, 'property_type' );
								break;
							...
							case 'floor_plan' :
							case 'property_photo' :
								$first_image = !empty($value) ? $value[0] : null;

                                if (!empty($first_image)) {
                                    set_post_thumbnail($property_id, $first_image);
                                }
								$value = implode( ',', $value );
